...
const MAX_HEALTH = 2
const HEALTH_BOOST = 0.043
const HEALTH_PENALTY_OVERTAP = -0.0475
const HEALTH_PENALTY_MISS = -0.0975
const HEALTH_PENALTY_KE_BAD = -0.0475
const HEALTH_PENALTY_KE_SHIT = -0.0775
...
#		end_level_part_2()
	on_update(delta)
	if (!Conductor.counting_down and !Conductor.stream == null):
		update_health((-0.00025*60)*delta)
